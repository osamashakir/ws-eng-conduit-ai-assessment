...
const ASSESSMENT_TYPE = 'defect-resolution';
const ASSESSMENT_VERSION = 'v5';
const ASSESSMENT_BRANCH = `rwa/defect-resolution-${ASSESSMENT_VERSION}`;
...
async function createGitDiff(): Promise<string> {
  ...
  const diffOutput = execCommand(
    `git diff ${remoteName}/${ASSESSMENT_BRANCH}...HEAD -- . ":!root-causes.md" ":!*.patch" ":!yarn.lock" ":!package-lock.json" ":!**/tsconfig*.json"`,
  );
...
async function getSubmissionWarnings(diffPath: string): Promise<string[]> {
  ...
  try {
    const rcaStats = await fs.stat('root-causes.md');
    if (rcaStats.size < 100) {
      warnings.push('⚠️ WARNING: Root Cause Analysis - root-causes.md is very small. A thorough root cause analysis is required for a good evaluation score.');
    }
  } catch {
    warnings.push('⚠️ WARNING: Root Cause Analysis - No root-causes.md found. Missing root cause analysis will result in 0 stars for Completeness.');
  }
...
async function addRca(zip: JSZip): Promise<void> {
  try {
    await addFileToZip(zip, 'root-causes.md', 'root-causes.md');
  } catch {
    console.warn('⚠️ WARNING: No root-causes.md file found in root directory');
  }
}
